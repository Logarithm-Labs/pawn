# EVMAgenticWorkflow Template Prompt

Write an EVMAgenticWorkflow that implements an agentic graph using the langgraph-supervisor framework. This system should include three agents with the following responsibilities:

1. Swap Tokens: An agent that swaps tokens on EVM chains.
2. Mint NFT: An agent that mints NFTs on EVM chains.
3. Transfer Tokens: An agent that handles token transfers on EVM chains.
...
- **Swap Token Agent:**  
  ...
  - Responsibility: Swap tokens on EVM chains using GOAT tools and an abstract Uniswap plugin.
  - Prompt should describe its role as a token swapping expert.

- **Mint NFT Agent:**  
  - Function: mint_nft()
  - Responsibility: Mint NFTs on EVM chains using GOAT tools and an abstract Crossmint plugin.
  - Prompt should describe its role as an NFT minting expert.

- **Transfer Tokens Agent:**  
  - Function: send_receive_tokens()
  - Responsibility: Transfer tokens on EVM chains using GOAT tools and an abstract transfer tool (e.g., send_eth).
  - Prompt should describe its role as a token transfer expert.

Also, ensure that the supervisor is configured to integrate these three agents with a prompt that clearly delegates tasks for token swapping, NFT minting, and token transferring.

Your final output should be only the complete Python code for EVMAgenticWorkflow that follows the above structure and incorporates the three agent functionalities as specified.
